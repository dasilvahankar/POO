...
<div id="wrapper">
	...
	<section id="box">
		...
		<?php
			echo 'Je suis l\'objet <b>'.Vehicule::getNombre().'</b> qui n\'existe pas encore!<br/><br/>'; //fonctione SI SEULEMENT la fonction est STATIQUE
			
			//création d'un objet de type Vehicule( poids , vitesse max , couleur , prix )
			$vroom = new Vehicule( 80 , 240 , '#d1ceb2' , 8500 );
			//echo '<b>'.$vroom->getNombre().'</b><br/>';			
			$vroom->afficher();		
			
			//création d'un objet de type Voiture( nombre de roues , consommation , poids , vitesse max , couleur , prix )			
			$voiture = new Voiture( 4 , 6 , 850 , 240 , '#00a0e9' , 18000 );
			//echo '<b>'.$voiture->getNombre().'</b><br/>';
			$voiture->afficher();
			
			$voiture = new Voiture( 6 , 12 , 1250 , 90 , '#cfdb00' , 46500 );
			//echo '<b>'.$voiture->getNombre().'</b><br/>';
			$voiture->afficher();			
