<?php
	...
	include_once('lib/php/connection.class.php');		
	include_once('lib/php/voiture.class.php');

	//connection à la BD
	//$connection = new Connection( $bd,$sgbd,$serveur,$login,$mdp ); //Fonctionne aussi
	$connection = new Connection( $bd ); //OK	
?>
	...
	<section id="box">
					...
					<tr>
						...
						<td>
							...
							<?php	
								...
								$resultats = $connection->Select( $sql );
		...
		<?php
			...
			if( isset($_POST['btnAfficher']) || isset($_POST['btnEnregistrer']) )
			{
				...
				{	
					//préparation du INSERT
					$sql = 'INSERT INTO voiture VALUES(null,"'.$voiture->Proprietaire().'",'.$voiture->MarqueID().','.$voiture->Reservoir().','.$voiture->Consommation().',"'.$voiture->Plaque().'")';
					
					//appel de la méthode qui va faire le INSERT
